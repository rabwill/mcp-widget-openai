...
// Load the OpenAI Apps SDK widget HTML
const widgetHtmlPath = resolve(process.cwd(), "public/claims-widget.html");
let widgetHtml: string;
try {
  widgetHtml = readFileSync(widgetHtmlPath, "utf8");
  console.log(`Loaded OpenAI Apps SDK widget from ${widgetHtmlPath}`);
} catch (e) {
  console.error(`Failed to load widget HTML from ${widgetHtmlPath}:`, e);
    ...
    resources: [
      {
        ...
        description: 'Interactive dashboard for viewing and managing Zava Insurance claims. Displays data in cards with statistics, status badges, and action buttons.',
        _meta: {
...
app.post('/mcp/messages', async (req, res) => {
  try {
    ...
    switch (method) {
      case 'initialize':
        result = {
          ...
          serverInfo: {
            name: 'zava-claims-mcp-server',
            version: '1.0.0',
            description: 'Zava Insurance Claims MCP Server with OpenAI Apps SDK widget support'
          }
        };
        break;
      ...
      // OpenAI Apps SDK resource handlers
      case 'resources/list':
        result = {
          resources: [
            {
              ...
              name: 'claims-widget',
              mimeType: 'text/html+skybridge',
              description: 'Interactive dashboard for viewing and managing Zava Insurance claims.',
              _meta: {
                'openai/widgetPrefersBorder': true,
                'openai/widgetDescription': 
                  'Interactive dashboard for viewing and managing Zava Insurance claims, inspections, contractors, and inspectors. ' +
